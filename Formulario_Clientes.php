...
function getClientes($conn, $filtro = '', $tipo_filtro='') {
    ...
    if (!empty($filtro)) {
        ...
        else if ($tipo_filtro=='rnc')
        $sql.=" WHERE rnc LIKE '%$filtro%'";
        else if ($tipo_filtro=='telefono')
        $sql.=" WHERE telefono LIKE '%$filtro%'";
        else if ($tipo_filtro=='ciudad')
        $sql .=" WHERE id_ciudad LIKE '%$filtro%'";
    }
    $result = $conn->query($sql);
    return $result->fetch_all(MYSQLI_ASSOC);
}
...
$filtro = isset($_POST['filtro_clientes']) ? $_POST['filtro_clientes'] : '';
$tipo_filtro=isset($_POST['Tipo_F']) && $_POST['Tipo_F'] !=='default' ? $_POST['Tipo_F'] : 'default';
#echo "<script>alert('$tipo_filtro') </script>";
$clientes = getClientes($conn,  $filtro,$tipo_filtro);
...
<body>
    ...
    <div id="clientes-lista">
        <div style="border-bottom: 2px solid#ccc;">
            <form id="Filt" action="Formulario_Clientes.php" method="post" style="border:0;">
                <label for="filtro_clientes">Filtrar Clientes:</label>
                <input type="text" id="filtro_clientes" name="filtro_clientes" value= "<?= $filtro; ?>" placeholder="Buscar cliente...">
                <select name="Tipo_F" id="Tipo_F" required>
                <option value="default" name="default" >Tipo de filtro</option>
                <option value="telefono" name="telefono" <?php if(isset($_POST['Tipo_F']) && $_POST['Tipo_F'] == 'telefono') echo 'selected'; ?>>Telefono</option>
                <option value="nombre" name="nombre" <?php if(isset($_POST['Tipo_F']) && $_POST['Tipo_F'] == 'nombre') echo 'selected'; ?>>Nombre</option>
                <option value="ciudad" name="ciudad" <?php if(isset($_POST['Tipo_F']) && $_POST['Tipo_F'] == 'ciudad') echo 'selected'; ?>>Ciudad</option>
                <option value="rnc" name="rnc" <?php if(isset($_POST['Tipo_F']) && $_POST['Tipo_F'] == 'rnc') echo 'selected'; ?>>Rnc</option>
                </select><br>
